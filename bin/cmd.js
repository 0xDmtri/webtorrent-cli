...
const options = {
  ...
  simple: {
    o: {
      alias: "out",
      desc: "Set download destination",
      type: "string",
      requiresArg: true,
    },
    s: {
      alias: "select",
      desc: "Select specific file in torrent",
      defaultDescription: "List files",
    },
    i: {
      alias: "interactive-select",
      desc: "Interactively select specific file in torrent",
      type: "boolean",
    },
    t: {
      alias: "subtitles",
      desc: "Load subtitles file",
      type: "string",
      requiresArg: true,
    },
  },
  advanced: {
    p: {
      alias: "port",
      desc: "Change the http server port",
      type: "number",
      default: 8000,
      requiresArg: true,
    },
    b: {
      alias: "blocklist",
      desc: "Load blocklist file/url",
      type: "string",
      requiresArg: true,
    },
    a: {
      alias: "announce",
      desc: "Tracker URL to announce to",
      type: "string",
      requiresArg: true,
    },
    q: { alias: "quiet", desc: "Don't show UI on stdout", type: "boolean" },
    d: {
      alias: "download-limit",
      desc: "Maximum download speed in kB/s",
      type: "number",
      requiresArg: true,
      default: -1,
    ...
    u: {
      alias: "upload-limit",
      desc: "Maximum upload speed in kB/s",
      type: "number",
      requiresArg: true,
      default: -1,
      defaultDescription: "unlimited",
    ...
    pip: {
      desc: "Enter Picture-in-Picture if supported by the player",
      type: "boolean",
    },
    verbose: { desc: "Show torrent protocol details", type: "boolean" },
    playlist: {
      desc: "Open files in a playlist if supported by the player",
      type: "boolean",
    },
    "player-args": {
      desc: "Add player specific arguments (see example)",
      type: "string",
      requiresArg: true,
    },
    "torrent-port": {
      desc: "Change the torrent seeding port",
      defaultDescription: "random",
      type: "number",
      requiresArg: true,
    },
    "dht-port": {
      desc: "Change the dht port",
      defaultDescription: "random",
      type: "number",
      requiresArg: true,
    },
    "not-on-top": {
      desc: 'Don\'t set "always on top" option in player',
      type: "boolean",
    },
    "keep-seeding": {
      desc: "Don't quit when done downloading",
      type: "boolean",
    },
    "no-quit": { desc: "Don't quit when player exits", type: "boolean" },
    quit: { hidden: true, default: true },
    "on-done": {
      desc: "Run script after torrent download is done",
      type: "string",
      requiresArg: true,
    },
    "on-exit": {
      desc: "Run script before program exit",
      type: "string",
      requiresArg: true,
    },
  },
...
const commands = [
  {
    command: ["download [torrent-ids...]", "$0"],
    desc: "Download a torrent",
    handler: (args) => {
      processInputs(args.torrentIds, runDownload);
    },
  },
  {
    command: "downloadmeta <torrent-ids...>",
    desc: "Download metadata of torrent",
    handler: (args) => {
      processInputs(args.torrentIds, runDownloadMeta);
    },
  },
  {
    command: "seed <inputs...>",
    desc: "Seed a file or a folder",
    handler: (args) => {
      processInputs(args.inputs, runSeed);
    },
  },
  {
    command: "create <input>",
    desc: "Create a .torrent file",
    handler: (args) => {
      runCreate(args.input);
    },
  },
  {
    command: "info <torrent-id>",
    desc: "Show torrent information",
    handler: (args) => {
      runInfo(args.torrentId);
    },
  },
  {
    command: "version",
    desc: "Show version information",
    handler: () => yargs.showVersion("log"),
  },
...
process.on("exit", (code) => {
  ...
  console.log(
    chalk`{green OPEN AN ISSUE:} https://github.com/webtorrent/webtorrent-cli/issues\n`,
  );
  console.log(
    `DEBUG INFO: webtorrent-cli ${webTorrentCliVersion}, webtorrent ${webTorrentVersion}, node ${process.version}, ${process.platform} ${process.arch}, exit ${code}`,
  );
});
...
yargs
  .command(commands)
  .options(options.streaming)
  .group(Object.keys(options.streaming), "Options (streaming): ")
  .options(options.simple)
  .group(
    Object.keys(options.simple).concat(["help", "version"]),
    "Options (simple): ",
  )
  .options(options.advanced)
  .group(Object.keys(options.advanced), "Options (advanced)");
...
yargs
  ...
  .version(
    ...
    `${webTorrentCliVersion} (${webTorrentVersion})`,
  )
...
function init(_argv) {
  ...
  const selectedPlayers = Object.keys(argv).filter((v) =>
    Object.keys(options.streaming).includes(v),
  );
...
async function runDownload(torrentId) {
  ...
  torrent.on("infoHash", () => {
    ...
    function updateMetadata() {
      console.clear();
      console.log(
        chalk`{green fetching torrent metadata from} {bold ${torrent.numPeers}} {green peers}`,
      );
    }
  ...
  torrent.on("done", () => {
    torrentCount -= 1;
    if (!argv.quiet) {
      const numActiveWires = torrent.wires.reduce(
        (num, wire) => num + (wire.downloaded > 0),
        0,
      );
  ...
  server.listen(argv.port).on("error", (err) => {
  ...
  async function onReady() {
    ...
    if (argv["interactive-select"] && torrent.files.length > 1) {
      ...
      const answers = await inquirer
        ...
        .catch((err) => {
          if (err.isTtyError) {
            return errorAndExit(
              "Could not render interactive selection mode in this terminal.",
            );
          } else {
            return errorAndExit(
              "Could not start interactive selection mode: " + err,
            );
          }
        });
    ...
    const index =
      typeof argv.select === "number"
        ? argv.select
        : torrent.files.indexOf(
  ...
  async function onSelection(index) {
    href =
      argv.airplay || argv.chromecast || argv.xbmc || argv.dlna
    ...
    if (
      ...
      torrent.files.forEach((file) =>
        allHrefs.push(new URL(href + file.streamURL).toString()),
      );
      // set the first file to the selected index
      allHrefs = allHrefs
        .slice(index, allHrefs.length)
        .concat(allHrefs.slice(0, index));
    } else {
    ...
    if (argv.vlc) {
      ...
      argv.playlist
        ? openPlayer(playerArgs.mplayer.concat(allHrefs))
      ...
      argv.playlist
        ? openPlayer(playerArgs.smplayer.concat(allHrefs))
        : openPlayer(playerArgs.smplayer.concat(JSON.stringify(href)));
    }

    function openPlayer(args) {
      cp.spawn(JSON.stringify(args[0]), args.slice(1), {
        stdio: "ignore",
        shell: true,
      })
        .on("error", (err) => {
    ...
    if (argv.xbmc) {
      ...
      new xbmc.Browser().on("deviceOn", (device) =>
        device.play(href, () => {}),
      );
    }
...
function runDownloadMeta(torrentId) {
  ...
  torrent.on("infoHash", function () {
    ...
    torrent.on("metadata", function () {
      ...
      console.clear();
      console.log(
        chalk`{green saving the .torrent file data to ${torrentFilePath} ...}`,
      );
...
function runSeed(input) {
  if (
    path.extname(input).toLowerCase() === ".torrent" ||
    /^magnet:/.test(input)
  ) {
...
function drawTorrent(torrent) {
  ...
  function draw() {
    ...
    line(
      chalk`{green ${seeding ? "Seeding" : "Downloading"}:} {bold ${torrent.name}}`,
    );
    ...
    if (argv["torrent-port"])
      portInfo.push(chalk`{green Torrent port:} ${argv["torrent-port"]}`);
    if (argv["dht-port"])
      portInfo.push(chalk`{green DHT port:} ${argv["dht-port"]}`);
    if (portInfo.length) line(portInfo.join(" "));

    if (playerName) {
      line(
        chalk`{green Streaming to:} {bold ${playerName}}  {green Server running at:} {bold ${href}}`,
      );
    } else if (server) {
    ...
    line(
      chalk`{green Speed:} {bold ${prettierBytes(
        speed,
      )}/s} {green Downloaded:} {bold ${prettierBytes(
        torrent.downloaded,
      )}}/{bold ${prettierBytes(torrent.length)}} {green Uploaded:} {bold ${prettierBytes(
        torrent.uploaded,
      )}}`,
    );

    line(
      chalk`{green Running time:} {bold ${
        runtime
      }}  {green Time remaining:} {bold ${estimate}}  {green Peers:} {bold ${
        unchoked.length
      }/${torrent.numPeers}}`,
    );
    ...
    torrent.wires.every((wire) => {
      ...
      if (argv.verbose) {
        ...
        const reqStats = wire.requests.map((req) => req.piece);
